...
for dataset in ${DATASETS[@]}; do
    ...
    for num_seed in $(seq 0 $((num_seeds-1))); do
        ...
        train_data_path=outputs/prompts/$model/$dataset/all.jsonl
        ...
        for loss in $losses_options; do
            ...
            # Predict on val
            base_predictions_dir="outputs/adaptation/$model/lora_${loss}_plus_dp_cal/$list_identifier"
            ...
            # Predict on test
            test_output_dir="$base_predictions_dir/uncal-$test_list"
...
for num_seed in $(seq 0 $((num_seeds-1))); do
    ...
    for loss in $losses_options; do
        ...
        for dataset in ${DATASETS[@]}; do
            ...
            base_predictions_dir="outputs/adaptation/$model/lora_${loss}_plus_dp_cal/$list_identifier"
            predictions_list="val_${total_size}_${val_prop}_$num_seed"
            predictions_dir="$base_predictions_dir/uncal-$dataset-$predictions_list"
            if [ ! -f $predictions_dir/logits.csv ]; then
                mkdir -p $predictions_dir
                export CUDA_VISIBLE_DEVICES=0,1
                python -m llmcal2.scripts.run_posteriors \
            ...
            test_list="test_${dataset2testsize[$dataset]}"
            test_data_path=outputs/prompts/$model/$dataset/all.jsonl
            test_output_dir="$base_predictions_dir/uncal-$dataset-$test_list"
            if [ ! -f $test_output_dir/logits.csv ]; then
                mkdir -p $test_output_dir
                export CUDA_VISIBLE_DEVICES=0,1
                python -m llmcal2.scripts.run_posteriors \
