        ...
        for size in ${dataset2samples[$dataset]}; do
            for random_state in ${dataset2seed[$dataset"_"$size]}; do
                for method in $methods; do
                    ...
                    output_dir="outputs/adaptation/$model/lora_plus_$method/$dataset/size=$size/rs=$random_state"
                    if [ $method == "dp_calibration" ]; then
                        method_old="affine_scalar"
                    elif [ $method == "temp_scaling" ]; then
                        method_old="temp_scaling"
                    elif [ $method == "bias_only" ]; then
                    ...
                    if [ ! -f $output_dir/train_logits.csv ]; then
                        ...
                        #     --input_dir ../llmcal/experiments/$dataset"_"$size"_"$random_state/basic_$dataset"_0-shot_litgpt"/lm_$model/no_adaptation_bf16/$method_old/predictions/train \
                        #     --output_dir $output_dir \
                        #     --split "train"
                        # python -m llmcal2.scripts.temp_copy_results \
                        #     --input_dir ../llmcal/experiments/$dataset"_"$size"_"$random_state/basic_$dataset"_0-shot_litgpt"/lm_$model/no_adaptation_bf16/$method_old/predictions/validation \
                        #     --output_dir $output_dir \
                        #     --split "validation"
                        # python -m llmcal2.scripts.temp_copy_results \
                        #     --input_dir ../llmcal/experiments/$dataset"_"$size"_"$random_state/basic_$dataset"_0-shot_litgpt"/lm_$model/no_adaptation_bf16/$method_old/predictions/test \
                        #     --output_dir $output_dir \
                        #     --split "test"
                        # cp ../llmcal/experiments/$dataset"_"$size"_"$random_state/basic_$dataset"_0-shot_litgpt"/lm_$model/no_adaptation_bf16/$method_old/checkpoint/model.pth $output_dir/checkpoint/model.pth
                        # cp ../llmcal/experiments/$dataset"_"$size"_"$random_state/basic_$dataset"_0-shot_litgpt"/lm_$model/no_adaptation_bf16/$method_old/logs/events* $output_dir/logs/
                        # cp ../llmcal/experiments/$dataset"_"$size"_"$random_state/basic_$dataset"_0-shot_litgpt"/lm_$model/no_adaptation_bf16/$method_old/logs/metrics.csv $output_dir/logs/metrics.csv
                        # cp ../llmcal/experiments/$dataset"_"$size"_"$random_state/basic_$dataset"_0-shot_litgpt"/lm_$model/no_adaptation_bf16/$method_old/*.ckpt $output_dir/
                    fi
                done
            done
        done
