...
def apply_calibration(logits, alpha, beta, encoded_answers):
    mask = torch.zeros(logits.size(0), device=logits.device, dtype=torch.bool)
    mask[encoded_answers] = True
    logits = logits.masked_fill(~mask, -float("inf"))
    beta_expanded = torch.zeros(logits.size(0), device=logits.device, dtype=logits.dtype)
    beta_expanded[encoded_answers] = beta
    logits = torch.log_softmax(logits * alpha + beta_expanded, dim=-1)
    return logits
...
def generate_fn(
    ...
    *,
    ...
    for current_idx in range(encoded_answers.size(1)):

        # Generate the token
        token = next_token(model, input_pos, token.view(1, -1), alpha, beta / encoded_answers.size(1), encoded_answers[:,current_idx], temperature=temperature, top_k=top_k, top_p=top_p)
        tokens.append(token)
