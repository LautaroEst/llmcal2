...
def create_prompt_for_generative_model(df, config):
    df = df.copy()
    df["answer"] = [config["answers_templates"] for _ in range(len(df))]
    df["prompt"] = df["text"].apply(lambda x: config["prompt_template"].format(text=x))
...
def main(
    ...
    if template_type == "generative":
        for key in data:
            data[key] = create_prompt_for_generative_model(data[key], prompt_template_config)
    else:
